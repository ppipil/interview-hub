...
class SqlitePersistenceRepository:
  ...
  def seed_formal_questions(self, questions: List[FormalQuestionBankWrite]) -> None:
    if not questions:
      return
    with self._connect() as connection:
      self._insert_formal_questions(connection, questions, replace_existing=False)

  def replace_interviewer_question_bank(self, interviewer_id: str, questions: List[FormalQuestionBankWrite]) -> None:
    with self._connect() as connection:
      connection.execute(
        "DELETE FROM formal_question_bank WHERE scope_type = ? AND interviewer_id = ?",
  ...
  def replace_global_question_bank(self, role: InterviewRole, questions: List[FormalQuestionBankWrite]) -> None:
    with self._connect() as connection:
      connection.execute(
        "DELETE FROM formal_question_bank WHERE scope_type = ? AND role = ?",
        ("global", role),
      )
      self._insert_formal_questions(connection, questions, replace_existing=False)
  ...
  def list_formal_questions(
    ...
    *,
    ...
    if interviewer_id:
      query.append("AND interviewer_id = ?")
      params.append(interviewer_id)
    if role:
      query.append("AND role = ?")
      params.append(role)
    if stage_key:
      query.append("AND stage_key = ?")
  ...
  def save_formal_question_usage(self, usage: FormalQuestionUsageEntry) -> None:
    with self._connect() as connection:
      connection.execute(
        """
        INSERT OR REPLACE INTO formal_question_usage (
  ...
  def _row_to_formal_question(self, row: sqlite3.Row) -> FormalQuestionBankEntry:
  ...
  def _row_to_formal_question_usage(self, row: sqlite3.Row) -> FormalQuestionUsageEntry:
