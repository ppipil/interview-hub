...
一个用于 AI 模拟面试的 monorepo。当前版本已经打通了“配置面试 -> 实时答题 -> 反馈复盘”的主链路，并补上了隐藏管理台、面试官问卷、数据库题库和多 provider 骨架。
...
- 后端题库已切到数据库唯一出题源：
  - 第一题走通用开场题
  - 中间轮次统一走专业题库
  - 专业题优先使用面试官专属题库，不足时回退到通用题库
  - 总轮次大于等于 `3` 时，最后一题走通用收尾题
- 隐藏管理能力已完成第一版：
  - 面试官 CRUD
...
- 当前后端默认题库已经按“通用开场题 + 专业题 + 通用收尾题”入库
- 后端岗位默认专业题库已按 6 组主题补齐：
  - Java / 语言基础
  - MySQL
  - Redis
  - 计算机网络与系统
...
- 前端可以独立部署到 Vercel / Netlify，但必须显式配置 `VITE_API_BASE_URL`
- 后端当前更适合部署到常驻服务环境，或先通过本地隧道做演示联调
- `DATABASE_URL` 配置后支持 SQLite / Neon(Postgres) 持久化
- SecondMe 与豆包的真实联调仍依赖测试凭证和外部网络环境
